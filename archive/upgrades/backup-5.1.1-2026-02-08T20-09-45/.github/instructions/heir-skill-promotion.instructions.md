...
## Post-Upgrade Verification Protocol

After running `Alex: Upgrade Architecture`, verify migration success:

### Verification Checklist

| Check | Command/Action | Expected |
|-------|----------------|----------|
| **Manifest version** | Read `.github/config/alex-manifest.json` | `version` matches target |
| **Skill count** | Count `.github/skills/` subdirectories | Same or greater than before |
| **Episodic memories** | Count `.github/episodic/` files | All preserved |
| **Global knowledge** | Check `~/.alex/global-knowledge/` | Intact (user-level, not project-specific) |
| **User profile** | Read `.github/config/user-profile.json` | Project settings preserved |

### Quick Verification Commands (PowerShell)

```powershell
# Version check
(Get-Content .github/config/alex-manifest.json | ConvertFrom-Json).version

# Skill count
(Get-ChildItem .github/skills -Directory).Count

# Episodic count
(Get-ChildItem .github/episodic).Count
```

### Post-Verification Cleanup

After confirming successful migration:
1. **Remove backup** if no longer needed: `Remove-Item archive/upgrades -Recurse -Force`
2. **Commit clean state**: `git add -A && git commit -m "chore: verified upgrade to vX.X.X"`

**Synapse**: [dream-state-automation.instructions.md] (0.7, validates, forward) - "Dream command provides additional synapse validation"
